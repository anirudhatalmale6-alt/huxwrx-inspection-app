#!/usr/bin/env python3
"""v218: Safe OnSuccess — NO new statements, only email HTML modifications.

Key insight from v216/v217 testing:
- v216 (no OnSuccess changes) = works perfectly
- v217 (added Patch + Refresh statements) = same error
- Conclusion: ANY new statement in OnSuccess breaks it

Strategy for v218:
- OnSuccess: ONLY modify existing email HTML to add Quantity rows (no new statements)
- Hold Reason: Auto-set via DataCard Default formula (same pattern as Release Confirmation)
- Everything else: Same popup/AQL/visibility changes
"""
...
OUTPUT = os.path.join(PROJECT_DIR, 'v218_aql.msapp')
