#!/usr/bin/env python3
"""v213: Fix popup (2 fields + visible buttons), un-hide Notes, hide Justification input,
fix Level I minor sample sizes, add txt_QTY control.

Source: June 21 export #2 (has v210 auto-hold + AQL fixes baked in).
Includes all v211/v212 changes plus v213 fixes.
"""
...
OUTPUT = os.path.join(PROJECT_DIR, 'v213_aql.msapp')
...
def build():
    ...
    # --- Y1. OnSuccess Patch: Add Hold Reason ---
    old = ('{DispositionStatus: If(gblIsFail, {Value: "On Hold"}, {Value: "Accepted"}), '
           "'Notes / Observations':")
    new = ('{DispositionStatus: If(gblIsFail, {Value: "On Hold"}, {Value: "Accepted"}), '
           '\'Hold Reason\': If(gblIsFail, {Value: "Hold for 100% Sorting"}, Blank()), '
           "'Notes / Observations':")
    assert old in yaml_text, 'Y1 fail'
    yaml_text = yaml_text.replace(old, new, 1); changes += 1
    print('Y1: Hold Reason auto-set')
    ...
    # --- Y13. Add gblQTYText + clear in hold popup ---
    # Clear txt_QTY on popup close (in the release branch and hold branch)
    # Already using gblReset which resets txt_QTY too
    ...
    os_old = ("{DispositionStatus: If(gblIsFail, {Value: \"On Hold\"}, {Value: \"Accepted\"}), "
              "'Notes / Observations':")
    os_new = ("{DispositionStatus: If(gblIsFail, {Value: \"On Hold\"}, {Value: \"Accepted\"}), "
              "'Hold Reason': If(gblIsFail, {Value: \"Hold for 100% Sorting\"}, Blank()), "
              "'Notes / Observations':")
    assert os_old in os_script, 'J1a fail'
    os_script = os_script.replace(os_old, os_new, 1)
