#!/usr/bin/env python3
"""v219: v218 + Release Confirmation fix + QTY label readability.

Fixes from v218 testing:
- Release Confirmation Default overrides stored value (always shows "Failed inspection
  do not release" when outcome=Fail, even after hold release). Fix: use stored value
  for existing records, conditional default only for new records.
- QTY DataCard label hard to read on mobile — bump font size.
"""
...
OUTPUT = os.path.join(PROJECT_DIR, 'v219_aql.msapp')
...
def build():
    ...
    new_hr_default = ("- Hold Reason_DataCard1:\n"
                      ...
                      "                                          BorderColor: =RGBA(245, 245, 245, 1)\n"
                      "                                          DataField: =\"HoldReason\"\n"
                      "                                          Default: =If(DataCardValue25.Selected.Value = \"Fail\", {Value: \"Hold for 100% Sorting\"}, ThisItem.'Hold Reason')\n")
    ...
    set_rule(hr_dc, "Default", 'If(DataCardValue25.Selected.Value = "Fail", {Value: "Hold for 100% Sorting"}, ThisItem.\'Hold Reason\')', "Data")
