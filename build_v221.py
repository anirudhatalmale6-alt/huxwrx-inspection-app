#!/usr/bin/env python3
"""v221: Fix Hold Reason circular auto-fill after release.

v220 issue: Blank() clears Hold Reason in SharePoint, but Default formula sees
it's blank + outcome=Fail → re-fills "Hold for 100% Sorting". Fix: also check
DispositionStatus — don't auto-fill if already released (Accepted).
"""
...
OUTPUT = os.path.join(PROJECT_DIR, 'v221_aql.msapp')
...
def build():
    ...
    # --- Y1. OnSuccess: Add Quantity row to REGULAR email (modify existing HTML only) ---
    ...
    assert reg_idx != -1, 'Y1a fail'
    pos = yaml_text.find(reg_email_old, reg_idx)
    assert pos != -1, 'Y1b fail'
    ...
    changes += 1
    print('Y1: Regular email Quantity row')

    # --- Y2. OnSuccess: Add Quantity row to AUTO-HOLD email (modify existing HTML only) ---
    ah_marker = "Inspection Auto-Hold (AQL Fail):"
    ah_idx = yaml_text.find(ah_marker)
    assert ah_idx != -1, 'Y2 fail'
    ah_id = "<tr><td style='font-weight:bold;'>ID</td><td>\" & Text(Form1.LastSubmit.ID) & \"</td></tr>"
    ah_pos = yaml_text.find(ah_id, ah_idx)
    assert ah_pos != -1, 'Y2b fail'
    ah_new = (ah_id +
              "<tr><td style='font-weight:bold;'>Quantity</td><td>\" & Text(Form1.LastSubmit.'Quantity Recieved') & \"</td></tr>")
    yaml_text = yaml_text[:ah_pos] + ah_new + yaml_text[ah_pos + len(ah_id):]
    changes += 1
    print('Y2: Auto-hold email Quantity')

    # --- Y3. Hold Reason DataCard Default: auto-select on Fail ---
    old_hr_default = ("- Hold Reason_DataCard1:\n"
                      ...
                      "                                          Default: =ThisItem.'Hold Reason'\n")
    new_hr_default = ("- Hold Reason_DataCard1:\n"
                      "                                        Control: TypedDataCard@1.0.7\n"
                      "                                        Variant: ClassicComboBoxEdit\n"
                      "                                        IsLocked: true\n"
                      "                                        Properties:\n"
                      "                                          BorderColor: =RGBA(245, 245, 245, 1)\n"
                      "                                          DataField: =\"HoldReason\"\n"
                      "                                          Default: =If(IsBlank(ThisItem.'Hold Reason'), If(And(DataCardValue25.Selected.Value = \"Fail\", ThisItem.DispositionStatus.Value <> \"Accepted\"), {Value: \"Hold for 100% Sorting\"}), ThisItem.'Hold Reason')\n")
    assert old_hr_default in yaml_text, 'Y3 fail'
    yaml_text = yaml_text.replace(old_hr_default, new_hr_default, 1); changes += 1
    print('Y3: Hold Reason auto-select on Fail')
    ...
    # --- Y9. Release Patch: txt_QTY.Text + Release Confirmation ---
    old_rel = (
    ...
    # --- Y15. Release Confirmation Default: show stored value for existing records ---
    old_rc_default = ('                                          Default: |-\n'
                      '                                            =If(DataCardValue25.Selected.Value = "Fail", {Value: "Failed inspection do not release"}, {Value: "Released to inventory / warehouse location"})\n'
                      '                                          DisplayName: =DataSourceInfo([@\'Incoming/In process & Final Inspections\'],DataSourceInfo.DisplayName,\'Release Confirmation\')')
    new_rc_default = ('                                          Default: =If(IsBlank(ThisItem.\'Release Confirmation\'), If(DataCardValue25.Selected.Value = "Fail", {Value: "Failed inspection do not release"}, {Value: "Released to inventory / warehouse location"}), ThisItem.\'Release Confirmation\')\n'
                      '                                          DisplayName: =DataSourceInfo([@\'Incoming/In process & Final Inspections\'],DataSourceInfo.DisplayName,\'Release Confirmation\')')
    assert old_rc_default in yaml_text, 'Y15 fail'
    yaml_text = yaml_text.replace(old_rc_default, new_rc_default, 1); changes += 1
    print('Y15: Release Confirmation Default fixed')

    # --- Y16. QTY DataCard label: override DisplayName for readability ---
    old_qty_dn = ("- 100% inspection total QTY passed_DataCard1:\n"
                  "                                        Control: TypedDataCard@1.0.7\n"
                  "                                        Variant: ClassicTextualEdit\n"
                  "                                        IsLocked: true\n"
                  "                                        Properties:\n"
                  "                                          BorderColor: =RGBA(245, 245, 245, 1)\n"
                  "                                          DataField: =\"OData__x0031_00_x0025_inspectiontotalQ\"\n"
                  "                                          Default: =ThisItem.'100% inspection total QTY passed'\n"
                  "                                          DisplayName: =DataSourceInfo([@'Incoming/In process & Final Inspections'],DataSourceInfo.DisplayName,'OData__x0031_00_x0025_inspectiontotalQ')")
    new_qty_dn = ("- 100% inspection total QTY passed_DataCard1:\n"
                  "                                        Control: TypedDataCard@1.0.7\n"
                  "                                        Variant: ClassicTextualEdit\n"
                  "                                        IsLocked: true\n"
                  "                                        Properties:\n"
                  ...
                  "                                          DataField: =\"OData__x0031_00_x0025_inspectiontotalQ\"\n"
                  "                                          Default: =ThisItem.'100% inspection total QTY passed'\n"
                  "                                          DisplayName: =\"QTY Passed (100% Inspection)\"")
    assert old_qty_dn in yaml_text, 'Y16 fail'
    yaml_text = yaml_text.replace(old_qty_dn, new_qty_dn, 1); changes += 1
    print('Y16: QTY DataCard DisplayName shortened')

    yaml_data = yaml_text.encode('utf-8')
    print(f'\nYAML: {changes} changes applied')

    # ============================================================
    # JSON CHANGES (with CRLF output)
    # ============================================================

    # --- J1. OnSuccess: ONLY modify email HTML (no new statements!) ---
    form1 = find_control(json_data, "Form1")
    assert form1, 'J1 fail'
    os_script = get_rule(form1, "OnSuccess")

    # Quantity in auto-hold email (modify existing HTML string only)
    ...
    if ah_pos_j != -1:
        ...
        if ah_id_pos != -1:
            ...
            print('J1a: Auto-hold email Quantity in JSON')

    # Quantity in regular email (modify existing HTML string only)
    ...
    if reg_pos_j != -1:
        ...
        if reg_id_pos != -1:
            ...
            print('J1b: Regular email Quantity in JSON')

    set_rule(form1, "OnSuccess", os_script, "Behavior")
    print('J1: OnSuccess updated (email Qty only, NO new statements)')

    # --- J2. Hold Reason DataCard Default: auto-select on Fail ---
    hr_dc = find_control(json_data, "Hold Reason_DataCard1")
    assert hr_dc, 'J2 fail'
    set_rule(hr_dc, "Default", 'If(IsBlank(ThisItem.\'Hold Reason\'), If(And(DataCardValue25.Selected.Value = "Fail", ThisItem.DispositionStatus.Value <> "Accepted"), {Value: "Hold for 100% Sorting"}), ThisItem.\'Hold Reason\')', "Data")
    print('J2: Hold Reason auto-select in JSON')

    # --- J3. DataCardValue23: Visible=false ---
    dcv23 = find_control(json_data, "DataCardValue23")
    assert dcv23, 'J3 fail'
    set_rule(dcv23, "Visible", "false", "Design")
    print('J3: DataCardValue23 hidden')

    # --- J4. Headerlbl_41 Text ---
    h41 = find_control(json_data, "Headerlbl_41")
    set_rule(h41, "Text", f'If({IS_ON_HOLD}, "Release Hold", "Hold Request")', "Data")
    print('J4: Header dynamic')

    # --- J5. lbl_Director_Comments_6 Text ---
    ...
    print('J5: Label dynamic')

    # --- J6. txt_Reason: Height, Y, HintText ---
    ...
    print('J6: txt_Reason conditional')

    # --- J7. Add txt_QTY control ---
    ...
    print('J7: txt_QTY added to JSON')

    # --- J8. btn_Submit_17 OnSelect: update release + hold Patches ---
    btn = find_control(json_data, "btn_Submit_17")
    sel = get_rule(btn, "OnSelect")

    # Release Patch: add Release Confirmation + QTY field
    old_j_rel = (
        "DispositionStatus: {Value: \"Accepted\"},\n"
        "            DispositionDate: Today(),\n"
        "            DispositionReason: If(IsBlank(RecordsGallery1.Selected.DispositionReason),\n"
    ...
    assert old_j_rel in sel, 'J8a fail'
    ...
    if rel_idx_j != -1:
        ...
        if rp != -1:
            ...
            print('J8b: Release email QTY row')
    ...
    if hs_idx_j != -1:
        ...
        if hp != -1:
            ...
            print('J8c: Hold email Quantity row')

    set_rule(btn, "OnSelect", sel, "Behavior")
    print('J8: btn_Submit_17 OnSelect updated')

    # --- J9. AQL: Fix minSS Level I ---
    dck24 = find_control(json_data, "DataCardKey24")
    assert dck24, 'J9 fail'
    aql_formula = get_rule(dck24, "Text")
    assert aql_formula, 'J9b fail'
    ...
    assert old_minss in aql_formula, 'J9c fail'
    aql_formula = aql_formula.replace(old_minss, new_minss, 1)
    print('J9: minSS Level I fixed in JSON')

    # --- J10. AQL: Fix majSS Level I ---
    ...
    assert old_majss in aql_formula, 'J10 fail'
    aql_formula = aql_formula.replace(old_majss, new_majss, 1)
    print('J10: majSS Level I fixed in JSON')

    # --- J11. AQL: Fix Ac formula ---
    ...
    assert ac_count_j == 2, f'J11 fail: expected 2, found {ac_count_j}'
    aql_formula = aql_formula.replace(old_ac_j, new_ac_j)
    print(f'J11: Ac formula fixed in JSON ({ac_count_j} occurrences)')

    set_rule(dck24, "Text", aql_formula, "Data")

    # --- J12. Release Confirmation Default: show stored value for existing records ---
    rc_dc = find_control(json_data, "Release Confirmation_DataCard1")
    assert rc_dc, 'J12 fail'
    set_rule(rc_dc, "Default",
             'If(IsBlank(ThisItem.\'Release Confirmation\'), If(DataCardValue25.Selected.Value = "Fail", {Value: "Failed inspection do not release"}, {Value: "Released to inventory / warehouse location"}), ThisItem.\'Release Confirmation\')',
             "Data")
    print('J12: Release Confirmation Default fixed in JSON')

    # --- J13. QTY DataCard DisplayName shortened ---
    qty_dc = find_control(json_data, "100% inspection total QTY passed_DataCard1")
    assert qty_dc, 'J13 fail'
    set_rule(qty_dc, "DisplayName", '"QTY Passed (100% Inspection)"', "Data")
    print('J13: QTY DataCard DisplayName shortened in JSON')

    # ============================================================
    # Serialize JSON with CRLF (critical!)
    # ============================================================
