#!/usr/bin/env python3
"""v220: v219 + Hold Reason clears on release + admin override for auto-set fields.

Fixes:
- Hold Reason clears (Blank) when hold is released via popup
- Hold Reason Default uses stored value for existing records (admin can override)
- Both auto-set fields only force values for NEW records, not existing ones
"""
...
OUTPUT = os.path.join(PROJECT_DIR, 'v220_aql.msapp')
...
def build():
    ...
    new_hr_default = ("- Hold Reason_DataCard1:\n"
                      ...
                      "                                          BorderColor: =RGBA(245, 245, 245, 1)\n"
                      "                                          DataField: =\"HoldReason\"\n"
                      "                                          Default: =If(IsBlank(ThisItem.'Hold Reason'), If(DataCardValue25.Selected.Value = \"Fail\", {Value: \"Hold for 100% Sorting\"}), ThisItem.'Hold Reason')\n")
    ...
    set_rule(hr_dc, "Default", 'If(IsBlank(ThisItem.\'Hold Reason\'), If(DataCardValue25.Selected.Value = "Fail", {Value: "Hold for 100% Sorting"}), ThisItem.\'Hold Reason\')', "Data")
